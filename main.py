...
def restart(room, player_hit_points, player_rect, Sword_rect, sword_rect, PLAYER_rect):
    ...
    return room, player_hit_points, player_rect, Sword_rect, sword_rect, PLAYER_rect
#Subtracts hitpoints
def hitpoints(player_hit_points, room):
    global PLAYER_rect
    if player_hit_points == 3:
        player_hit_points -= 1
...
boss = pygame.image.load(
    'Game_Assets/Character_Sprites/lava_boss.png').convert_alpha()
...
restart_door = pygame.image.load(
    'Game_Assets/Map_Tiles/door(restart).png').convert_alpha()
...
enemies = [[[Enemy(130, 370, goblin_enemy, img_width, img_height), [3, 3]],
           ...
           [[
               Enemy(100, 400, boss, boss_img_width, boss_img_height, 'Boss'),
               [5, 5]
           ]]]
game_maps = [
    ...
    if enemies_remaining == True:
        for e in range(len(enemies[room])):
            ...
            if enemy_object.rect.colliderect(PLAYER_rect):
                player_hit_points = hitpoints(player_hit_points, room)
            if player_hit_points == 0:
                white = (255, 255, 255)
                red = (255, 0, 0)
                # X = 500
    ...
    for event in pygame.event.get():
        ...
        if event.type == KEYDOWN:
            ...
            if event.key == pygame.K_r and player_hit_points == 0:
              restart(room, player_hit_points, player_rect, Sword_rect, sword_rect, PLAYER_rect)
